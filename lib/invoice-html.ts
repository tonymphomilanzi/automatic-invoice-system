// lib/invoice-html.ts  — replace entire file
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .company-sub{font-size:11px;color:#555;margin-top:3px;line-height:1.6}
  .invoice-title{font-size:16px;font-weight:700;color:#333;margin-top:8px;letter-spacing:2px}
  .meta{display:flex;justify-content:space-between;background:#f0f4f8;
        padding:14px 18px;border-radius:6px;margin-bottom:20px;font-size:12px}
  ...
  .footer{margin-top:30px;text-align:center;font-size:10px;
          color:#888;border-top:1px solid #eee;padding-top:14px}
...
<div class="header">
  ...
  <div class="company-sub">
    ${addressLine}${addressLine && phoneLine ? " &nbsp;|&nbsp; " : ""}${phoneLine}
    ${s.email ? `&nbsp;|&nbsp; ${s.email}` : ""}
    ${s.vatNumber ? `<br>VAT Reg: ${s.vatNumber}` : ""}
  </div>
...
<div class="meta">
  <div>
    ...
    ${s.vatNumber ? `<p><strong>VAT Reg:</strong> ${s.vatNumber}</p>` : ""}
  </div>
  <div style="text-align:right">
    <p><strong>To:</strong> ${s.defaultBillTo}</p>
    <p><strong>Payment Terms:</strong> ${s.paymentTerms}</p>
  </div>
</div>
<table>
  <thead>
  ...
  <tbody>
    <tr>
      ...
      <td class="right">${s.currencySymbol} ${formatMoney(data.rate)}</td>
      <td class="right">${s.currencySymbol} ${formatMoney(data.amount)}</td>
    </tr>
    ...
    <tr class="subtotal-row">
      <td colspan="9" class="right">VAT (${vatPct}%):</td>
      <td class="right">${s.currencySymbol} ${formatMoney(data.vat)}</td>
    </tr>
    <tr class="total-row">
      <td colspan="9" class="right">TOTAL (${s.currencySymbol}):</td>
...
${
  s.invoiceFooterNote
    ? `<div style="margin-top:16px;padding:10px 14px;background:#f8fafc;
                  border-radius:5px;font-size:11px;color:#444;border-left:3px solid #1e3a5f">
        ${s.invoiceFooterNote}
       </div>`
    : ""
}
<div class="footer">
  Generated on ${new Date().toLocaleDateString("en-GB")} — ${s.companyName}
</div>
<script>
